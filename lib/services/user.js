...
module.exports = class UserService extends Service {
    ...
    async addFilmToFavorites(userId, filmId) {
        try {
            const { User } = this.server.models();
            await User.query().findById(userId).patch({ $addToSet: { favoriteFilms: filmId } });
        } catch (error) {
    ...
    async getUsersWhoFavoritedFilm(filmId) {
        try {
            const { User } = this.server.models();
            return await User.query().where('favoriteFilms', 'contains', filmId);
        } catch (error) {
            console.error('Error fetching users who favorited film', error);
            throw this.boomify(error);
        }
    }
